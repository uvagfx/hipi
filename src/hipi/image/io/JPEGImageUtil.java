...
public class JPEGImageUtil implements ImageDecoder, ImageEncoder {
    ...
    if (raster.getNumBands() == 4) {
      ...
      for (int i = 0; i < h; i++)
        for (int j = 0; j < w; j++) {
          ...
          pels[i * w * 3 + j * 3] =
              (float) ((Math.min(Math.max(Y + 1.772f * (Cb - 128), 0), 255)) / 255.0);
          pels[i * w * 3 + j * 3 + 1] =
              (float) ((Math.min(Math.max(Y - 0.34414f * (Cb - 128) - 0.71414f * (Cr - 128), 0),
                  255)) / 255.0);
          pels[i * w * 3 + j * 3 + 2] =
              (float) ((Math.min(Math.max(Y + 1.402f * (Cr - 128), 0), 255)) / 255.0);
        }
  ...
  public void encodeImage(FloatImage image, ImageHeader header, OutputStream os) throws IOException {
    ...
    for (int i = 0; i < (image.getWidth() * image.getHeight()) / (4 - image.getBands()); i++) {
