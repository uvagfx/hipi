...
/**
 * Base class for FloatImage's. A FloatImage is just an array of floating-point values along with
 * information about the dimensions of the image that array represents. The is the default image
 * type that is used in HIPI.
 * 
...
public class FloatImage implements Writable, RawComparator<BinaryComparable> {
   ...
   * 
   * @return A {@link FloatImage} of the converted image. Returns null if the image could not be
   *         converted
   */
  ...
  /**
   * This method comes from the RawComparator class and allows sorting to happen much faster than in
   * the normal Comparable interface. For a discussion of this, see Hadoop: The Definitive Guide.
   * Essentially, this method avoids deserializing the entire FloatImage object before doing a
   * comparison. Since the first bytes indicate the size of the image, we can just read a small
   * segment of the byte array to get the sizes.
   * 
   * TODO: Ensure that the second and fifth parameters are actually defining the start
   */
