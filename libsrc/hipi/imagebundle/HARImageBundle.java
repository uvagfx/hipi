...
public class HARImageBundle extends AbstractImageBundle {
  ...
  private void closeIndex() throws IOException {
    ...
    // hash values must be sorted before writing indexes (normally this is done automatically by
    // MapReduce)
    ...
    for (; i < indexHash.size(); i++) {
      ...
      if (i > 0 && i % 1000 == 0) {
        ...
        endIndexHash = hash;
        String masterWrite =
            startIndexHash + " " + endIndexHash + " " + startPos + " " + indexStream.getPos()
                + " \n";
    ...
    if (i > 0 && i % 1000 > 0) {
      String masterWrite =
          startIndexHash + " " + hash + " " + startPos + " " + indexStream.getPos() + " \n";
      masterIndexStream.write(masterWrite.getBytes());
    }
    // close the streams
